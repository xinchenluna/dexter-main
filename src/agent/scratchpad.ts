...
/** Meta-tools that trigger nested LLM routing — hard-capped per query */
export const HARD_LIMIT_TOOLS = new Set(['get_financials', 'get_market_data']);
 ...
 * 
 * Audit log and compaction input for a query (LLM context lives in ToolMessages).
 *
 * Tool limits:
 * - Soft warnings for most tools (similarity + approaching limit)
 * - Hard block for meta-tools in HARD_LIMIT_TOOLS after maxCallsPerTool
 */
export class Scratchpad {
  ...
  canCallTool(toolName: string, query?: string): { allowed: boolean; warning?: string } {
    ...
    if (HARD_LIMIT_TOOLS.has(toolName) && currentCount >= maxCalls) {
      return {
        allowed: false,
        warning: `Tool '${toolName}' reached the per-query limit (${maxCalls} calls). ` +
          `Use data already retrieved, try read_filings/web_search, or answer with noted gaps.`,
      };
    }

    // Over soft limit — warn but allow (non meta-tools only reach here below max)
  ...
  getActiveToolResultCount(): number {
    return this.readEntries().filter((e) => e.type === 'tool_result').length;
  }
